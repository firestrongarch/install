...
- Python层：安装器负责解析包清单、执行下载与安装、执行配置类任务，**新增代理检测与自动切换功能**
- Shell层：交换分区脚本负责系统级内存扩展配置
- GUI层（Qt）：提供可视化选择与命令执行界面
- 配置层：YAML清单描述软件包元数据与安装方式，**包含代理配置结构**
...
- [install.py:138-181](file://install.py#L138-L181)
...
## 架构总览
下图展示了从用户交互到系统命令执行的关键流程，涵盖Python安装器与Qt GUI两种入口，**新增代理检测与自动切换流程**。
...
- [install.py:55-88](file://install.py#L55-L88)
- [install.py:138-181](file://install.py#L138-L181)
- [install.py:183-197](file://install.py#L183-L197)
- [packages.yaml:1-58](file://packages.yaml#L1-L58)
...
## 结论
本指南围绕Install项目的安装器、GUI与系统配置组件，提供了从环境准备、日志分析到问题定位与解决的完整流程。**新增的代理检测与自动切换功能显著提升了网络连接的可靠性**，建议在生产环境中结合重试、超时与回滚机制，确保安装过程的稳定性与可恢复性；同时通过GUI与Python双入口，满足不同用户的使用习惯。
